...
Такое впечатление, что придется таки лезть в миграции руками и разводить старое и новое. Строго говоря, 
нового у нас всего два поля в `library.Book`. Вот наша начальная миграция на момент создания:
```python
...
```
Нам нужно сделать так, чтобы в схему базы добавились поля `abstract`. И еще у нас есть поле `useless_field`. 
С ним получилось странно, потому что по итогам предыдущих экспериментов оно почему-то добавилось как read only, 
а нам надо, чтобы оно было нормальным. Поэтому дропнем его от греха подальше в базе и создадим заново.
```sql
alter table library_book drop column useless_field;
```
Теперь создадим пустую миграцию в `library`:
```bash
python manage.py makemigrations --name abstract_useless_field library --empty
```
Вот что получилось:
```python
...
```
А также удаляем эти поля их начальной миграции во избежание последующего бардака:
```python
# Generated by Django 4.0.5 on 2022-06-01 17:43
...
        migrations.CreateModel(
...
Operations to perform:
  Apply all migrations: admin, auth, blog, contenttypes, library, sessions
Running migrations:
  Applying library.0002_abstract_useless_field... OK
```
И у нас в базе появились эти поля. Правда почему-то всё-таки опять read only.

Отбой, это не read only, а глюки ГУИ. Всё в порядке, метод работает.